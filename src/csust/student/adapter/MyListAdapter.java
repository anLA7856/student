...
public class MyListAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			hold.teacherPic = (ImageView) convertView
					.findViewById(R.id.itemTeacherPic);
			hold.teacherNum = (TextView) convertView
					.findViewById(R.id.itemTeacherNum);
			hold.courseName = (TextView) convertView
					.findViewById(R.id.itemCourseName);
			hold.teacherName = (TextView) convertView
					.findViewById(R.id.itemTeacherName);
		...
		if (list.get(position).getTeacherNum().equalsIgnoreCase("")) {
			...
			hold.teacherPic.setTag(Model.USERHEADURL
					+ list.get(position).getTeacherNum());
			Bitmap bitTeacher = loadImgHeadImg.loadImage(hold.teacherPic,
					Model.USERHEADURL + list.get(position).getTeacherNum(),
					new ImageDownloadCallBack() {
						@Override
						public void onImageDownload(ImageView imageView,
								Bitmap bitmap) {
							if (position >= list.size()) {
								if (hold.teacherPic.getTag().equals(
										Model.USERHEADURL
												+ list.get(position - 1)
														.getTeacherNum())) {
