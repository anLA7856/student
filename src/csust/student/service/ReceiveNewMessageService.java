			...
			if (msg.what == 404) {
				...
				for(int i = 0;i < list.size();i++){
					...
					if(Model.MYCHATACTIVITY == null){
						...
						if(cm.getSenderId() == Integer.parseInt(Model.MYCHATACTIVITY.getTeacherId().toString())
								&& cm.getReceiveId() == Model.MYUSERINFO.getStudent_id()){
			...
			public void run() {
				while(true){
					...
					if(!isRun){
						...
						url =  Model.STUGETNEWCHATMESSAGE+"studentId="+Model.MYUSERINFO.getStudent_id();
						Log.i("service", url);
